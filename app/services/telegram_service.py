...
def _post_with_retry(payload, retries=3, delay=2):
    ...
    for attempt in range(1, retries + 1):
        try:
            response = requests.post(TELEGRAM_URL, json=payload, proxies=PROXIES, timeout=10)
...
def send_order_confirmation(order_id, chat_id=DEFAULT_CHAT_ID):
    with get_connection() as connection:
        cursor = connection.cursor()
        cursor.execute("""
             SELECT om.created_on, od.item_name_snapshot, od.quantity, od.unit_price, od.line_total
            FROM order_details od, order_master om
    ...
    if not items:
        return {"status": "error", "message": f"No items found for Order {order_id}"}

    raw_created_on = items[0][0]
    try:
        order_date = datetime.fromisoformat(str(raw_created_on)).strftime("%d %b %Y")
    except (TypeError, ValueError):
        order_date = str(raw_created_on).split(" ")[0]

    item_lines = [
        f"  • *{item_name_snapshot}* x {quantity} = ৳ {line_total:.2f}"
        for created_on, item_name_snapshot, quantity, unit_price, line_total in items
    ]
    items_text = "\n".join(item_lines)
